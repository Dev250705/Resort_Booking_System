...
export default function ResortDetails() {
  ...
  const loadReviewEligibility = useCallback(async () => {
    ...
    try {
      const r = await fetch(
        `http://localhost:5000/api/reviews/eligibility/${id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleBookNow = (room) => {
    ...
    if (searchDates.checkIn === searchDates.checkOut) {
      alert(
        "Enter valid dates: Check-in and Check-out cannot be the same day.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:5000/api/bookings/checkout",
        {
          ...
          body: JSON.stringify({
            resortId: id,
            roomTypeTitle: selectedRoomToBook.title,
      ...
      if (!razorpayKeyId)
        throw new Error("Razorpay key missing. Please configure payment keys.");
      ...
      const options = {
        ...
        description: `Booking for ${selectedRoomToBook.title}`,
        ...
        handler: async function (paymentResponse) {
          try {
            ...
            if (verifyRes.ok && verifyData.success) {
              ...
              setBookError("Payment verification failed.");
            }
          } catch (err) {
            setBookError("Network error during verification.");
          } finally {
            setBookLoading(false);
          }
        },
      ...
      razorpayObj.on("payment.failed", function (res) {
        setBookError(res.error.description || "Payment Failed.");
      });
      razorpayObj.open();
    } catch (err) {
      setBookError(err.message);
    } finally {
      if (!bookSuccess && !String(bookError).includes("MOCK"))
        setBookLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="resort-detail-page">
        <Navbar />
        <div
          style={{ textAlign: "center", padding: "100px", marginTop: "120px" }}
        >
  ...
  const prevResortImg = () => {
    if (!resort?.images) return;
    setCurrentResortImgIdx(
      (prev) => (prev - 1 + resort.images.length) % resort.images.length,
    );
  };
  ...
  return (
    <div className="resort-detail-page">
      ...
      <section className="resort-premium-header">
        <div className="resort-header-text">
          ...
          {!reviewsBlock.loading && (
            <p
              className="resort-review-summary"
              ...
              {reviewsBlock.reviewCount > 0 ? (
                <>
                  <span className="rrs-stars" aria-hidden>
                    ...
                    {"☆".repeat(
                      5 - Math.round(reviewsBlock.averageRating || 0),
                    )}
                  </span>
                  <span className="rrs-num">
                    {reviewsBlock.averageRating?.toFixed(1)}
                  </span>
                  <span className="rrs-count">
                    ({reviewsBlock.reviewCount} guest
                    {reviewsBlock.reviewCount === 1 ? "" : "s"})
                  </span>
                </>
              ) : (
                <span className="rrs-count">No ratings yet</span>
              )}
            </p>
          )}
        </div>
        <div
          className={`resort-gallery-grid count-${Math.min(resort.images?.length || 1, 3)}`}
        >
          <div
            className="gallery-main-img"
            onClick={() => {
              setCurrentResortImgIdx(0);
              setIsResortGalleryOpen(true);
            }}
          >
            <img
              src={
          ...
          <div
            ...
            onClick={() => setIsResortGalleryOpen(true)}
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
      ...
      <div className="detail-content-wrapper">
        <section className="resort-info-section">
          <div className="info-block">
            ...
            <p className="resort-desc-text">
              {resort.description ||
                "Escape to this luxury resort offering breathtaking views, modern amenities, and world-class service. Perfect for both relaxation and adventure."}
            </p>
          ...
          <div className="go-map-container">
            <iframe
              ...
              allowFullScreen
            />
            <div className="map-overlay-text">Location Map</div>
          </div>
        </section>
            ...
            >
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSearchDates((s) => ({ ...s, checkIn: e.target.value }))
                  }
                  style={{
                    padding: "8px",
              ...
              <div style={{ display: "flex", flexDirection: "column" }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSearchDates((s) => ({ ...s, checkOut: e.target.value }))
                  }
                  style={{
                    padding: "8px",
                    border: "1px solid #cbd5e1",
          ...
          <div className="rooms-grid">
            {resort.roomTypes?.length > 0 ? (
                ...
                <div className="room-card" key={room._id || room.title}>
                  ...
                  <div className="room-card-content">
                    ...
                    >
                      ...
                      {searchDates.checkIn && searchDates.checkOut ? (
                        <>
                          {room.availableInventory ?? room.inventoryCount ?? 0}{" "}
                          room
                        ...
                        <>
                          {room.inventoryCount ?? 2} room
                          {(room.inventoryCount ?? 2) === 1 ? "" : "s"} total
                        </>
                      )}
                    </div>
                    ...
                    <div className="room-card-footer">
                      <div className="room-card-price">
                        ...
                        <span>
                          + ₹
                          {Math.round(room.basePrice * 0.18).toLocaleString()}{" "}
                          taxes/night
                        </span>
                      </div>
                      <button
              ...
              <div className="no-rooms-msg">
                No room types available for this resort.
              </div>
            )}
          </div>
        </section>
        ...
        <section
          className="resort-reviews-section"
        ...
        >
          <div className="resort-review-write-card">
            ...
            <p className="review-write-sub">
              Everyone can read <strong>approved</strong> guest reviews below.
              Sign in to submit your own after a completed stay — new reviews
              are checked by our team before they appear publicly.
            </p>

            {!reviewWrite.loading && reviewWrite.pendingReview && (
              <p className="review-pending-banner" role="status">
                Thanks — your review is{" "}
                <strong>waiting for admin approval</strong>. It will show here
                once approved.
              </p>
            )}
            ...
            {!reviewWrite.loading && !authToken && (
              <div className="review-guest-cta">
                <p className="review-guest-note">
                  Sign in to rate this resort after your visit. Submission is
                  only available to guests with a completed stay.
                </p>
                <Link
            ...
            {!reviewWrite.loading &&
              ...
              !reviewWrite.pendingReview && (
                <p className="review-policy-inline">
                  You can submit a review here after a{" "}
                  <strong>confirmed</strong> stay at this resort, once check-out
                  has passed (11:00 AM on your check-out day).
                </p>
              )}

            {!reviewWrite.loading &&
              reviewWrite.eligible &&
              reviewWrite.bookingId && (
                <form
                  className="review-write-form"
                  onSubmit={submitResortPageReview}
                >
                  <h3 className="review-form-inner-title">Write your review</h3>
                  <label className="review-write-label">Rating</label>
                  <div
                    className="review-write-stars"
                    role="group"
                  ...
                  <label
                    className="review-write-label"
                    htmlFor="resort-review-comment"
                  >
                    Your review
                  </label>
                  <textarea
          ...
          {!reviewsBlock.loading &&
            ...
            reviewsBlock.reviewCount === 0 && (
              <p className="reviews-empty">
                No published reviews yet. Approved guest reviews will appear
                here.
              </p>
            )}
          {!reviewsBlock.loading &&
      ...
      {isModalOpen && selectedRoom && (
        <div className="go-modal-overlay" onClick={closeRoomModal}>
          <div
            className="go-modal-content"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="go-modal-header">
              <h2>{selectedRoom.title}</h2>
              <button className="go-modal-close" onClick={closeRoomModal}>
                &times;
              </button>
            </div>
            ...
            <div className="go-modal-body">
              <div className="go-modal-carousel">
                <img
                  src={
                    selectedRoom.images?.[currentImageIdx] ||
                    resort.images?.[0] ||
                    "https://images.unsplash.com/photo-1618773928121-c32242e63f39"
                  }
                  alt="Room"
                />
                <button className="go-m-slider-btn left" onClick={prevImg}>
                  &#10094;
                </button>
                <button className="go-m-slider-btn right" onClick={nextImg}>
                  &#10095;
                </button>
              </div>
              ...
              <div className="go-modal-specs">
                {selectedRoom.amenities && selectedRoom.amenities.length > 0 ? (
                  selectedRoom.amenities.map((feat, idx) => {
                    ...
                    if (t.includes("wifi") || t.includes("internet"))
                      ...
                      icon = "🛁";
                    else if (t.includes("coffee") || t.includes("tea"))
                      icon = "☕";
                    else if (t.includes("bed")) icon = "🛏️";
                    else if (t.includes("service") || t.includes("desk"))
                      icon = "🛎️";
                    else if (t.includes("view") || t.includes("balcony"))
                      icon = "🌅";
                  ...
                  <div className="spec-item" style={{ gridColumn: "1 / -1" }}>
                    ...
                    <span className="spec-text">
                      Standard premium amenities are included. Specific room
                      features will be updated by the administration shortly.
                    </span>
                  </div>
                )}
                ...
                {selectedRoom.maxGuests && (
                  <div className="spec-item">
                    <span className="spec-icon">👪</span>
                    <span className="spec-text">
                      Accommodates up to {selectedRoom.maxGuests} Guests
                    </span>
                  </div>
                )}
              </div>
      ...
      {isResortGalleryOpen && resort && (
        <div
          className="go-modal-overlay"
          onClick={() => setIsResortGalleryOpen(false)}
        >
          <div
            className="go-modal-content gallery-modal"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="go-modal-header">
              <h2>Property Photos</h2>
              <button
                className="go-modal-close"
                onClick={() => setIsResortGalleryOpen(false)}
              >
                &times;
              </button>
            ...
            {bookSuccess ? (
              ...
              <form className="dr-dw-form" onSubmit={submitBooking}>
                <div className="dr-dw-room-preview">
                  <img
                    src={selectedRoomToBook.images?.[0] || resort.images?.[0]}
                    alt="Room preview"
                  />
                  <div>
                    <strong>{selectedRoomToBook.title}</strong>
                    <span>
                      ₹{selectedRoomToBook.basePrice.toLocaleString()} / night
                    </span>
                  </div>
                </div>
                    ...
                    <div className="dr-stepper">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setQuantity((q) =>
                            Math.min(
                              selectedRoomToBook.availableInventory ??
                                selectedRoomToBook.inventoryCount ??
                                10,
                              q + 1,
                            ),
                          )
                    ...
                    <div className="dr-stepper">
                      ...
                      <button
                        ...
                        onClick={() =>
                          setGuests((g) =>
                            Math.min(
                              (selectedRoomToBook.maxGuests || 4) * quantity,
                              g + 1,
                            ),
                          )
                        }
                ...
                <div className="dr-dw-summary">
                  <div className="sum-row">
                    ...
                    <span>
                      ₹
                      {(
                        selectedRoomToBook.basePrice * quantity
                      ).toLocaleString()}
                    </span>
                  </div>
                  <div className="sum-row">
                ...
                <button
                  type="submit"
                  className="dr-dw-submit"
                  disabled={bookLoading || calculateTotal() <= 0}
                >
                  {bookLoading
                    ? "Processing Request..."
