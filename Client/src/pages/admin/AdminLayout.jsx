import { useState } from "react"; // Added useState for hamburger toggle
    ...
    <div className="admin-layout-root">
      <aside className="admin-sidebar">
        <div className="admin-sidebar-header">
          ...
          {/* Hamburger button (3 lines) - visible only on mobile */}
          <button 
            className="admin-hamburger" 
            onClick={() => setMenuOpen(!menuOpen)}
            aria-label="Toggle Navigation"
          >
            <span className={`admin-bar ${menuOpen ? 'active' : ''}`}></span>
            <span className={`admin-bar ${menuOpen ? 'active' : ''}`}></span>
            <span className={`admin-bar ${menuOpen ? 'active' : ''}`}></span>
          </button>
        </div>

        {/* Menu Wrapper - collapses on mobile, controlled by menuOpen */}
        <div className={`admin-sidebar-menu-wrapper ${menuOpen ? 'open' : ''}`}>
